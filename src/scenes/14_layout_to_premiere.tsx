...
import { Node, Rect, SVG } from '@motion-canvas/2d/lib/components';
import { Background } from '../components/Background';
import { Logo } from '../components/Logo';
import { Arrow } from '../components/Arrow';
import { fadeTransition } from '@motion-canvas/core/lib/transitions';
import microphoneSvg from '../components/media/microphone.svg?url';

export default makeScene2D(function* (view) {
  const backgroundRef = createRef<typeof Background>();
  const containerRef = createRef<Node>();
  const microphoneLogoRef = createRef<Logo>();
  const arrowRef = createRef<Arrow>();
  const premiereLogoRef = createRef<SVG>();
  ...
  view.add(
    <Node ref={containerRef}>
      {/* Left side: Microphone logo */}
      <Logo
        ref={microphoneLogoRef}
        x={-500}
        y={0}
        containerSize={450}
        logoWidth={250}
  ...
  // Animation sequence
  yield* waitUntil('layout_appears');
  
  // Show microphone logo first
  yield* microphoneLogoRef().opacity(1, 0.5);
  
  yield* waitFor(0.5);
  
  // Show arrow
  yield* arrowRef().fadeIn(0.5);
  
  yield* waitFor(0.5);
  
  // Show Premiere Pro logo
  yield* premiereLogoRef().opacity(1, 0.5);
