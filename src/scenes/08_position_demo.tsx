...
export default makeScene2D(function* (view) {
  ...
  const positionTextRef = createRef<StyledText>();
  const positionIconRef = createRef<Img>();
  ...
  // Add "Position" text (initially hidden)
  view.add(
    <StyledText
      ref={positionTextRef}
      colorType="black"
      size="lg"
      position={[positions[1].x + 280, positions[1].y]}
      opacity={0}
    />
  );

  // Add position icon (initially hidden)
  view.add(
    <Img
      ref={positionIconRef}
      src={positionIcon}
      ...
      width={80}
      height={80}
      position={[positions[1].x, positions[1].y]}
      scale={0}
      opacity={0}
    />
  );
